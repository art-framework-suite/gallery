...
art::TypeID getPartnerTypeID(TClass *tClass) {
  art::TypeID result;
  if (tClass) {
...
void DataGetterHelper::getByLabel(std::type_info const &typeInfoOfWrapper,
  ...
  if (inputTag.process().empty()) {
    ...
    if (itProcess != processNameToProcessIndex_.end()) {
      ...
      if (getBranchDataIndex(info.processIndexToBranchDataIndex(), processIndex,
                             branchDataIndex)) {
...
void DataGetterHelper::updateFile(TFile *iFile, TTree *iTree,
      ...
      if (getBranchDataIndex(old, processIndex, branchDataIndex)) {
        ...
        if (info.isAssns() && branch != nullptr) {
          ...
          if (typeIDInDescription != typeIDInBranchData) {
            ...
            branchDataVector_[branchDataIndex].reset(new AssnsBranchData(
                typeIDInDescription, tClass, branch, eventNavigator_, this,
...
void DataGetterHelper::updateEvent() { initializedForProcessHistory_ = false; }
...
DataGetterHelper::buildBranchName(InfoForTypeLabelInstance const &info,
  ...
  return art::canonicalProductName(info.type().friendlyClassName(),
                                   info.label(), info.instance(), processName);
}
...
void DataGetterHelper::addBranchData(std::string &&branchName,
  ...
  if (info.isAssns()) {
    ...
    art::TypeID typeIDInDescription(tClass->GetTypeInfo());
    branchDataVector_.emplace_back(new AssnsBranchData(
        typeIDInDescription, tClass, branch, eventNavigator_, this,
        std::move(branchName), info.type(), info.partnerType()));
  } else {
    branchDataVector_.emplace_back(new BranchData(info.type(), info.tClass(),
                                                  branch, eventNavigator_, this,
                                                  std::move(branchName)));
  }
  info.processIndexToBranchDataIndex().push_back(
      uupair(processIndex, branchDataIndex));
...
TClass *DataGetterHelper::getTClassUsingBranchDescription(
    unsigned int processIndex, InfoForTypeLabelInstance const &info) const {
...
DataGetterHelper::getInfoForTypeLabelInstance(
    art::TypeID const &type, std::string const &label,
...
bool DataGetterHelper::getBranchDataIndex(
  ...
  auto itBranchDataIndex = std::lower_bound(
      ...
      processIndexToBranchDataIndex.cend(), uupair(processIndex, 0),
...
void DataGetterHelper::updateBranchDataIndexOrderedByHistory(
...
bool DataGetterHelper::getByBranchDescription(
    art::BranchDescription const &desc, unsigned int &branchDataIndex) const {
  if (!initializedForProcessHistory_) {
    initializeForProcessHistory();
  }
...
DataGetterHelper::InfoForTypeLabelInstance::InfoForTypeLabelInstance(
    art::TypeID const &iType, std::string const &iLabel,
