...
namespace {
art::EDProduct *calculateEDProductAddress(TClass *const tClass, void *address) {
...
BranchData::BranchData(art::TypeID const &type, TClass *iTClass,
                       TBranch *iBranch, EventNavigator const *eventNavigator,
                       art::EDProductGetterFinder const *finder,
                       std::string &&iBranchName)
    : tClass_{iTClass}, address_{tClass_ != nullptr ? tClass_->New() : nullptr},
      edProduct_{calculateEDProductAddress(tClass_, address_)},
      branch_{iBranch}, eventNavigator_{eventNavigator}, finder_{finder},
...
art::EDProduct const *BranchData::uniqueProduct() const { return getIt(); }
