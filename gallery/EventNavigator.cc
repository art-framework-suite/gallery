...
EventNavigator::EventNavigator(std::vector<std::string> const &iFileNames)
    : fileNames_(iFileNames), numberOfFiles_(fileNames_.size()), fileEntry_(-1),
      firstFileWithEvent_(0), entriesInCurrentFile_(0), eventEntry_(0), file_(),
      eventsTree_(nullptr), eventAuxiliaryBranch_(nullptr), eventAuxiliary_(),
      pEventAuxiliary_(&eventAuxiliary_), previousEventAuxiliaryEntry_(-1),
      eventHistoryTree_(nullptr), eventHistoryBranch_(nullptr), eventHistory_(),
      pEventHistory_(&eventHistory_), previousEventHistoryEntry_(-1),
      historyMap_() {
...
void EventNavigator::nextFile() {
  ...
  if (file_) {
    std::cout << "Closing file, read " << file_->GetBytesRead() << " bytes in "
              << file_->GetReadCalls() << " transactions\n";
  }
...
TFile *EventNavigator::getTFile() const { return file_.get(); }

TTree *EventNavigator::getTTree() const { return eventsTree_; }

void EventNavigator::initializeTTreePointers() {
  eventsTree_ = dynamic_cast<TTree *>(
      file_->Get(art::rootNames::eventTreeName().c_str()));
