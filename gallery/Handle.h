...
template <class T> Handle<T>::Handle(T const *prod) : prod_{prod} {}

template <class T>
Handle<T>::Handle(std::shared_ptr<art::Exception const> iWhyFailed)
    : whyFailed_{iWhyFailed} {}

template <class T> inline T const &Handle<T>::operator*() const {
  return *product();
}
...
template <class T> inline T const *Handle<T>::product() const {
  if (!prod_)
    throwHandleWhyFailed_();
  return prod_;
}

template <class T> bool Handle<T>::isValid() const { return prod_ != nullptr; }

template <class T>
inline std::shared_ptr<art::Exception const> Handle<T>::whyFailed() const {
  return whyFailed_;
}

template <class T> void Handle<T>::throwHandleWhyFailed_() const {
  if (whyFailed_) {
    throw *whyFailed_;
  }
