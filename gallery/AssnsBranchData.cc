...
AssnsBranchData::
AssnsBranchData(art::TypeID const& type, TClass* iTClass, TBranch* iBranch, EventNavigator const* eventNavigator,
                art::EDProductGetterFinder const* finder, string&& iBranchName, art::TypeID const& infoType,
                art::TypeID const& infoPartnerType)
  : BranchData{type, iTClass, iBranch, eventNavigator, finder, move(iBranchName)}
{
  if (type == infoType) {
...
art::EDProduct const*
AssnsBranchData::
uniqueProduct_(art::TypeID const& wanted_wrapper_type) const
{
