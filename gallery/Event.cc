...
Event::Event(std::vector<std::string> const &fileNames, bool useTTreeCache,
...
long long Event::eventEntry() const { return eventNavigator_->eventEntry(); }

long long Event::fileEntry() const { return eventNavigator_->fileEntry(); }
...
bool Event::isValid() const { return eventNavigator_->isValid(); }

bool Event::atEnd() const { return eventNavigator_->atEnd(); }
...
void Event::next() { operator++(); }

void Event::previous() { operator--(); }

TFile *Event::getTFile() const { return eventNavigator_->getTFile(); }

TTree *Event::getTTree() const { return eventNavigator_->getTTree(); }
